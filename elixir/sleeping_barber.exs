defmodule ShopConfig do
  ...
  def arrival_interval_min,   do: 500
  def arrival_interval_max,   do: 2000
  def haircut_duration_min,   do: 1000
  def haircut_duration_max,   do: 4000
  def satisfaction_threshold, do: 3.0
...
defmodule ShopOwner do
  def run do
    ...
    spawn_customers(waiting_room_pid, 1, ShopConfig.total_customers())
    ...
    IO.puts("Total customers arrived:    #{ShopConfig.total_customers()}")
  ...
  defp spawn_customers(waiting_room_pid, current, total) do
    ...
    Process.sleep(Enum.random(ShopConfig.arrival_interval_min()..ShopConfig.arrival_interval_max()))
      ...
      :admitted ->
        ...
        receive do
          {:called_by_barber, _barber_pid} ->
            ...
            receive do
              {:rate_request, barber_pid2} ->
                wait_seconds = wait_ms / 1000.0
                jitter = Enum.random(-1..1)
                score = 5 - trunc(wait_seconds / ShopConfig.satisfaction_threshold()) + jitter
...
defmodule WaitingRoom do
  ...
  defp loop(queue, turned_away_count, barber_sleeping, barber_pid) do
    receive do
      {:arrive, customer_pid, customer_id} ->
        if length(queue) >= ShopConfig.waiting_room_capacity() do
          send(customer_pid, :turned_away)
          Log.log("WaitingRoom", "turned away Customer #{customer_id} (#{length(queue)}/#{ShopConfig.waiting_room_capacity()})")
          loop(queue, turned_away_count + 1, barber_sleeping, barber_pid)
        else
          send(customer_pid, :admitted)
          new_queue = queue ++ [{customer_pid, customer_id}]
          Log.log("WaitingRoom", "admitted Customer #{customer_id} (#{length(new_queue)}/#{ShopConfig.waiting_room_capacity()})")

          if barber_sleeping do
            Log.log("WaitingRoom", "waking up the barber")
            send(barber_pid, :wakeup)
            loop(new_queue, turned_away_count, false, barber_pid)
          else
            loop(new_queue, turned_away_count, barber_sleeping, barber_pid)
          end
        end
      ...
      {:next_customer, from_barber_pid} ->
        case queue do
          [{customer_pid, customer_id} | rest] ->
            Log.log("WaitingRoom", "sending Customer #{customer_id} to barber (queue: #{length(rest)}/#{ShopConfig.waiting_room_capacity()})")
            send(from_barber_pid, {:customer_ready, customer_pid, customer_id})
            loop(rest, turned_away_count, false, from_barber_pid)

          [] ->
            Log.log("WaitingRoom", "no customers waiting barber will sleep")
            send(from_barber_pid, :none_waiting)
            loop([], turned_away_count, true, from_barber_pid)
        end

      {:get_stats, from} ->
        send(from, {:wr_stats, turned_away_count, length(queue)})
        loop(queue, turned_away_count, barber_sleeping, barber_pid)
  ...
  defp loop(waiting_room_pid, cuts, avg_duration, avg_rating) do
    receive do
      {:customer_ready, customer_pid, customer_id} ->
        duration = Enum.random(ShopConfig.haircut_duration_min()..ShopConfig.haircut_duration_max())
